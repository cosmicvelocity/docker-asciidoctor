...
ENV IPA_EX_URL=http://dl.ipafont.ipa.go.jp/IPAexfont/IPAexfont00301.zip \
    ...
    NOTO_CJK_JP_URL=https://noto-website.storage.googleapis.com/pkgs/NotoSansCJKjp-hinted.zip

RUN apk add --no-cache --virtual .fonts-builddeps \
        curl \
    \
    && apk add --no-cache --repository http://dl-4.alpinelinux.org/alpine/edge/testing/ --allow-untrusted \
        font-noto \
    \
